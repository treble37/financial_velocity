defmodule FinancialVelocity.TaskModeler do
  ...
  def generate_report(initial_amount, monthly_savings, years, sample_size \\ 10000) do
    ...
    median = returns |> Statistics.median()
    ...
    IO.puts("Your median return was #{median}")
  end
  ...
  def compute_simulated_returns(initial_amount, monthly_savings, years, sample_size \\ 10000) do
    1..sample_size
    |> Enum.to_list()
    |> Task.async_stream(fn _x ->
      FinancialVelocity.calculate(
        initial_amount,
        UniformSampler.random_annual_return(),
        monthly_savings,
        years
      )
    end)
